...
function makeEmailLink(p) {
  ...
  return `mailto:${MERCHANT_EMAIL}?subject=${encodeURIComponent(subject)}&body=${body}`;
}
...
function renderProduct(p) {
  ...
  const images = Array.isArray(p.images) && p.images.length > 0 ? p.images : [p.image || "images/placeholder.png"];
  ...
  const thumbnailsHtml = images.map((imgSrc, index) =>
    `<img class="thumbnail ${index === 0 ? 'active' : ''}" src="${imgSrc}" alt="${escapeHtml(p.name)} - View ${index + 1}" data-index="${index}" loading="lazy" onerror="this.src='images/placeholder.png'">`
  ).join("");
  ...
  detailEl.innerHTML = `
    <div class="top">
      ...
      <div class="product-meta">
        ...
        <div class="price">
          ${p.old_price ? `<span class="old-price"><s>${formatINR(p.old_price)}</s></span>` : ''}
          <span class="current-price">${formatINR(p.price)}</span>
          ${p.old_price ? `<span class="discount">${Math.round(((p.old_price - p.price) / p.old_price) * 100)}% off</span>` : ''}
        </div>
